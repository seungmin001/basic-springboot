...
import com.example.smspring.springboot.config.auth.dto.OAuthAttributes;
...
import com.example.smspring.springboot.domain.user.UserRepository;
...
@Service
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> { //구글 로그인 이후 가져온 사용자의 정보를 이용해 가입 및 정보수정, 세션 저장 기능 지원
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String userNameAttributeName = userRequest.getClientRegistration().getProviderDetails()
                .getUserInfoEndpoint().getUserNameAttributeName(); //로그인 진행 시 키가 되는 필드값(Primary key). Google "sub"값 제공. 네이버/카카오 제공x
